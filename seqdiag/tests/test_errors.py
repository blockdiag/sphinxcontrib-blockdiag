...
class TestSphinxcontribSeqdiagErrors(unittest.TestCase):
    def setUp(self):
        self.app = FakeSphinx()
        sphinxcontrib.seqdiag.setup(self.app)
    ...
    @with_app(srcdir='docs/basic', confoverrides=dict(seqdiag_html_image_format='JPG'))
    ...
    @with_app(srcdir='docs/basic', confoverrides=dict(seqdiag_html_image_format='PDF'))
